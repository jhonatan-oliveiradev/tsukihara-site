...
test(
  "memory intro is cinematic, ordered, and unlocks interaction only after the reveal",
  async () => {
...
test(
  "reduced motion preserves the intro semantics without a long cinematic hold",
  async () => {
    const { getMemoryIntroSchedule } = await loadTimeline();
    assert.equal(typeof getMemoryIntroSchedule, "function");
