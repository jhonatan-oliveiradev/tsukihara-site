...
export function useHeroTimeline(rootRef: RefObject<HTMLElement | null>) {
  useEffect(() => {
    ...
    const mobilePointer = window.matchMedia(`(max-width: ${HERO_BREAKPOINTS.mobile}px)`).matches;
    const shell = root.closest<HTMLElement>(".ix-shell");

    const q = (selector: string) => select(selector) as HTMLElement[];
    const afterStates = q(".th-hero-state-after");
    const beforeStates = q(
    ...
    gsap.set(q("[data-mist-crimson]"), { opacity: 0 });
    gsap.set(q("[data-crimson-wash]"), { opacity: 0 });
    gsap.set(q("[data-character-rim]"), { opacity: 0 });
    gsap.set(q("[data-ground-glow]"), { opacity: 0 });
    gsap.set(q("[data-temple-light]"), { opacity: 0 });
    gsap.set(q("[data-moon-halo-crimson]"), { opacity: 0 });
    gsap.set(q("[data-right-petals-after]"), { opacity: 0 });
    gsap.set(q("[data-sakura-left]"), { opacity: 0.38 });
    gsap.set(q("[data-characters-before], [data-characters-after]"), {
      xPercent: 12,
      yPercent: 3,
      scale: 0.965,
      transformOrigin: "70% 85%",
    });
    gsap.set(q("[data-moon-shadow]"), { xPercent: -128, opacity: 0 });

    const ambient = gsap.timeline({ repeat: -1, yoyo: true, paused: reduced });
    if (!reduced) {
      ambient
        .to(q("[data-mist-before], [data-mist-after]"), {
          xPercent: 1.4,
          yPercent: -0.5,
          duration: 10,
          ease: "sine.inOut",
    ...
    if (reduced) {
      timeline
        .to(q("[data-sky-crimson]"), { opacity: 1, duration: 0.72 }, 0.14)
        .to(afterStates, { opacity: 1, duration: 0.72 }, 0.14)
        .to(beforeStates, { opacity: 0.14, duration: 0.72 }, 0.14)
        .to(q("[data-mist-crimson]"), { opacity: 0.58, duration: 0.55 }, 0.3)
        .to(q("[data-moon-halo-normal]"), { opacity: 0.1, duration: 0.55 }, 0.24)
        .to(q("[data-moon-halo-crimson]"), { opacity: 0.92, duration: 0.55 }, 0.3)
        .to(q("[data-crimson-wash]"), { opacity: 0.18, duration: 0.45 }, 0.42);
    } else {
      timeline
        .to(
          q("[data-characters-before], [data-characters-after]"),
          { xPercent: -0.8, yPercent: -0.3, scale: 1.004, duration: 0.095, ease: "power2.out" },
          0.01,
        )
        .to(
          q("[data-characters-before], [data-characters-after]"),
          { xPercent: 0, yPercent: 0, scale: 1, duration: 0.035, ease: "sine.out" },
          0.095,
        )
        ...
        .to(q("[data-temple-after]"), { opacity: 0.52, duration: 0.25 }, 0.31)
        .to(q("[data-temple-before]"), { opacity: 0.68, duration: 0.25 }, 0.31)
        .to(q("[data-mist-after]"), { opacity: 0.58, duration: 0.22 }, 0.3)
        .to(q("[data-mist-before]"), { opacity: 0.7, duration: 0.22 }, 0.3)
        .to(q("[data-sakura-right-after]"), { opacity: 0.5, duration: 0.22 }, 0.32)
        ...
        .to(q("[data-moon-before]"), { opacity: 0.18, duration: 0.2 }, 0.48)
        .to(q("[data-moon-halo-normal]"), { opacity: 0.08, duration: 0.18 }, 0.48)
        .to(q("[data-moon-halo-crimson]"), { opacity: 1, scale: 1.06, duration: 0.2 }, 0.48)
        .to(q("[data-temple-after]"), { opacity: 0.9, duration: 0.2 }, 0.48)
        .to(q("[data-temple-before]"), { opacity: 0.24, duration: 0.2 }, 0.48)
        .to(q("[data-ground-after]"), { opacity: 0.86, duration: 0.2 }, 0.49)
