...
const narrative = {
  pt: {
    ...
    completeLabel: "OS FRAGMENTOS REVELAM O TODO",
    details: [
      {
        label: "OLHAR",
  ...
  en: {
    ...
    completeLabel: "THE FRAGMENTS REVEAL THE WHOLE",
    details: [
      {
        label: "GAZE",
...
export function CharacterSpotlight({ locale }: CharacterSpotlightProps) {
  ...
  useLayoutEffect(() => {
    ...
    const tiles = gsap.utils.toArray<HTMLElement>("[data-akari-mosaic-tile]", root);
    ...
    const setPhase = (phase: number) => {
      const tilePhase = Math.min(tiles.length - 1, phase);
      const activeText = Math.min(items.length - 1, phase);
      const complete = phase >= tiles.length;

      tiles.forEach((tile, index) => {
        tile.classList.toggle("is-revealed", index <= tilePhase);
        tile.classList.toggle("is-current", index === tilePhase && !complete);
      });

      items.forEach((item, index) => {
        const active = index === activeText;
        item.hidden = !active;
        item.classList.toggle("is-active", active);
      });

      mosaicBoard?.classList.toggle("is-complete", complete);
      fullFigure?.classList.toggle("is-revealed", complete);
      completeLabel?.classList.toggle("is-visible", complete);
    };

    setPhase(0);

    if (reduced) {
      setPhase(tiles.length);
      return;
    }
    ...
    const ctx = gsap.context(() => {
      ...
      if (detailStage) {
        ScrollTrigger.create({
          ...
          onUpdate: ({ progress: value }) => {
            const phases = tiles.length + 1;
            const phase = Math.min(phases - 1, Math.floor(value * phases));
            setPhase(phase);
      ...
      if (mosaicBoard) {
        const onPointerMove = (event: PointerEvent) => {
          const rect = mosaicBoard.getBoundingClientRect();
          const x = event.clientX - rect.left;
          const y = event.clientY - rect.top;
          const rotateY = ((x / rect.width - 0.5) * 2.4).toFixed(2);
          const rotateX = ((0.5 - y / rect.height) * 1.8).toFixed(2);

          mosaicBoard.style.setProperty("--akari-cursor-x", `${x}px`);
          mosaicBoard.style.setProperty("--akari-cursor-y", `${y}px`);
          gsap.to(mosaicBoard, {
    ...
    <section ref={rootRef} id="akari" data-section className="akari-chapter">
      ...
      <div className="akari-editorial">
        ...
        <div className="akari-editorial__relic" aria-hidden="true">
          <Image src="/akari-details/detail_06.png" alt="" fill quality={100} sizes="28vw" />
        </div>
      </div>
      ...
      <div className="akari-details akari-details--mosaic" data-akari-details-stage>
        <div className="akari-details__sticky akari-mosaic-layout">
          <div className="akari-mosaic-stage">
            <div className="akari-mosaic-board" data-akari-mosaic-board>
              <div className="akari-mosaic-figure" data-akari-mosaic-figure>
                <Image
                  src="/akari-details/akari_detailed.jpg"
                  alt=""
                  fill
                  quality={100}
                  sizes="(max-width: 760px) 88vw, 24vw"
                />
                <span className="akari-mosaic-figure__veil" />
              </div>

              <div className="akari-mosaic-grid">
                {mosaicTiles.map((image, index) => (
                  ...
                  >
                    ...
                    />
                    <span className="akari-mosaic-tile__edge" />
                    <span className="akari-mosaic-tile__number">0{index + 1}</span>
                  </div>
                ))}
              </div>

              <div className="akari-mosaic-spotlight" aria-hidden="true" />
              <span className="akari-mosaic-complete" data-akari-complete-label>
                {beat.completeLabel}
              </span>
            </div>

            <div className="akari-mosaic-counter" aria-hidden="true">
              <span>01</span>
              <i />
              <span>06</span>
            </div>
          ...
          <div className="akari-details__content akari-mosaic-copy">
