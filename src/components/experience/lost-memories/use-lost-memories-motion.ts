...
export function useLostMemoriesMotion(rootRef: RefObject<HTMLElement | null>) {
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.utils.toArray<HTMLElement>("[data-archive-reveal]").forEach((header) => {
        gsap.fromTo(
          header,
          { opacity: 0, y: 12 },
          {
      ...
      gsap.utils.toArray<HTMLElement>("[data-archive-surface]").forEach((surface, index) => {
        gsap.fromTo(
          surface,
          { opacity: 0.72, y: index % 2 === 0 ? 14 : 10 },
          {
            opacity: 1,
            y: 0,
            duration: 1.05,
      ...
      gsap.utils.toArray<HTMLElement>('[data-archive-kind="photograph"]').forEach((photo) => {
      ...
      if (transition) {
        ...
        gsap.fromTo(
          words,
          { opacity: 0, x: (index) => (index === 0 ? -18 : 18) },
          {
            opacity: 1,
            x: 0,
            duration: 0.9,
